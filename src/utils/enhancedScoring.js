/**
 * Advanced Fuzzy Logic Scoring and Ranking Utilities
 * Implements comprehensive fuzzy logic algorithms for candidate evaluation
 */
 ...
 */
const calculateFuzzyScore = (value, targetValue, fuzzyFactor = 0.2) => {
  ...
  if (typeof value === 'number' && typeof targetValue === 'number') {
    // For numeric values, calculate normalized distance with threshold
 ...
 * @param {boolean} options.caseSensitive - Whether string comparisons should be case sensitive
 * @param {boolean} options.partial - Whether to allow partial matches for strings
 * @param {number} options.threshold - Similarity threshold for considering items as matches
 * @returns {number} A score between 0 and 1 representing the similarity
 */
function calculateArraySimilarity(sourceArray, targetArray, options = {}) {
  const { 
    caseSensitive = false, 
    partial = true, 
    threshold = 0.7 
  ...
  // Compute the optimal matching using a greedy approach
  ...
  // - Exact matching arrays will have score 1.0
  // - Arrays with no matches will have score 0.0
  // - Partially matching arrays will have a proportional score
...
/**
 * Fuzzify a crisp value into membership degrees for different sets
 * @param {Number} x - Crisp input value
 * @param {Object} membershipFunctions - Object with membership functions
 * @returns {Object} - Membership degrees for each set
 */
const fuzzifyValue = (x, membershipFunctions) => {
  const result = {};
  
  Object.keys(membershipFunctions).forEach(key => {
    const mf = membershipFunctions[key];
    
    if (mf.type === 'triangular') {
      result[key] = triangularMF(x, mf.a, mf.b, mf.c);
    } else if (mf.type === 'trapezoidal') {
      result[key] = trapezoidalMF(x, mf.a, mf.b, mf.c, mf.d);
    } else if (mf.type === 'boolean') {
      result[key] = x === mf.value ? 1 : 0;
    }
  });
 ...
 * @param {Array} points - Array of arrays with points for each membership function
 * @returns {Array} - Array of membership function definitions
 */
const defineTriangularMFs = (...points) => {
 ...
 * @returns {Array} - Array of membership function definitions
 */
const defineTrapezoidalMFs = (...points) => {
  return points.map((point, index) => ({
    type: 'trapezoidal',
    a: point[0],
    b: point[1],
...
/**
 * Apply fuzzy inference rules to input membership degrees
 * @param {Object} inputs - Input membership degrees
 * @param {Array} rules - Fuzzy inference rules
 * @returns {Array} - Rule outputs with firing strengths
 */
const applyFuzzyRules = (inputs, rules) => {
  const ruleOutputs = [];
  ...
  rules.forEach(rule => {
    ...
    // Process antecedents (IF part)
    rule.if.forEach(condition => {
      const { variable, is } = condition;
      const membershipDegree = inputs[variable]?.[is] || 0;
      
      // Use minimum for AND logic
      firingStrength = Math.min(firingStrength, membershipDegree);
    });
    
    // If rule fires (strength > 0), calculate output
    if (firingStrength > 0) {
      ruleOutputs.push({
        outputVariable: rule.then.variable,
        outputValue: rule.then.is,
 ...
 * @param {Array} fuzzyOutputs - Fuzzy rule outputs
 * @param {Object} outputMFs - Output membership functions
 * @param {Number} min - Minimum value in output range
 * @param {Number} max - Maximum value in output range
 * @param {Number} steps - Number of discretization steps
 * @returns {Number} - Defuzzified crisp value
 */
const defuzzifyCOG = (fuzzyOutputs, outputMFs, min, max, steps = 100) => {
  if (fuzzyOutputs.length === 0) return 0;
  
  // Create discrete evaluation points
  const stepSize = (max - min) / steps;
  let numerator = 0;
  let denominator = 0;
  
  for (let x = min; x <= max; x += stepSize) {
    // Calculate aggregate membership at this point
    let aggregateMembership = 0;
    
    fuzzyOutputs.forEach(output => {
      const mf = outputMFs[output.outputValue];
      let membership = 0;
      
      if (mf.type === 'triangular') {
        membership = triangularMF(x, mf.a, mf.b, mf.c);
      } else if (mf.type === 'trapezoidal') {
        membership = trapezoidalMF(x, mf.a, mf.b, mf.c, mf.d);
      }
      
      // Apply rule strength (min for Mamdani)
      membership = Math.min(membership, output.strength);
      
      // Max aggregation
      aggregateMembership = Math.max(aggregateMembership, membership);
    });
...
/**
 * Define linguistic variables for experience levels
 */
const experienceMFs = {
  junior: (x) => trapezoidalMF(x, 0, 0, 2, 3),
  midLevel: (x) => trapezoidalMF(x, 2, 3, 5, 6),
  senior: (x) => (x <= 4) ? 0 : Math.min((x - 4) / 2, 1)
};

/**
 * Enhanced rule evaluation with composite conditions
 * @param {Object} rule - Rule with conditions and consequence
 * @param {Object} fuzzifiedInputs - Fuzzified input values
 * @returns {Object} - Result with variable, value and strength
 */
const evaluateEnhancedRule = (rule, fuzzifiedInputs) => {
  let ruleStrength = null;
  
  for (const condition of rule.conditions) {
    const inputValue = fuzzifiedInputs[condition.variable][condition.is];
    const weightedValue = inputValue * (condition.weight || 1.0);
    
    if (ruleStrength === null) {
      ruleStrength = weightedValue;
    } else if (condition.operator === "AND") {
      ruleStrength = Math.min(ruleStrength, weightedValue);
    } else { // OR
      ruleStrength = Math.max(ruleStrength, weightedValue);
    }
  }
  
  return {
    variable: rule.consequence.variable,
    value: rule.consequence.value,
    strength: ruleStrength || 0
  };
};

/**
 * Mamdani fuzzy inference implementation
 * @param {Object} crispInputs - Raw input values
 * @param {Array} rules - Fuzzy rules
 * @param {Object} inputMFs - Input membership functions
 * @param {Object} outputMFs - Output membership functions
 * @returns {Object} - Aggregated fuzzy outputs
 */
const mamdaniInference = (crispInputs, rules, inputMFs, outputMFs) => {
  // Step 1: Fuzzify inputs
  const fuzzifiedInputs = {};
  Object.keys(crispInputs).forEach(variable => {
    fuzzifiedInputs[variable] = fuzzifyValue(crispInputs[variable], inputMFs[variable]);
  });
  
  // Step 2: Evaluate rules
  const ruleOutputs = rules.map(rule => evaluateEnhancedRule(rule, fuzzifiedInputs));
  
  // Step 3: Aggregate outputs
  const aggregatedOutputs = {};
  ruleOutputs.forEach(output => {
    if (!aggregatedOutputs[output.variable]) {
      aggregatedOutputs[output.variable] = {};
    }
    
    // For each linguistic value, take maximum of rule strengths
    if (!aggregatedOutputs[output.variable][output.value] || 
        aggregatedOutputs[output.variable][output.value] < output.strength) {
      aggregatedOutputs[output.variable][output.value] = output.strength;
    }
  });
 ...
 * @param {Object} fuzzyOutputs - Fuzzy rule outputs
 * @param {Object} outputMFs - Output membership functions
 * @param {Number} min - Minimum value in output range
 * @param {Number} max - Maximum value in output range
 * @param {Number} steps - Number of discretization steps
 * @returns {Number} - Defuzzified crisp value
 */
const defuzzifyMOM = (fuzzyOutputs, outputMFs, min, max, steps = 100) => {
  const stepSize = (max - min) / steps;
  let maxMembership = 0;
  let maxValues = [];
  ...
  for (let x = min; x <= max; x += stepSize) {
    ...
    // Calculate combined membership at this point
    for (const [term, strength] of Object.entries(fuzzyOutputs)) {
      if (outputMFs[term]) {
        const termMembership = Math.min(strength, outputMFs[term](x));
        membership = Math.max(membership, termMembership);
      }
    }
    ...
    if (membership > maxMembership) {
      ...
      maxValues = [x];
    } else if (Math.abs(membership - maxMembership) < 0.001) {
      maxValues.push(x);
    }
  }
  
  // Calculate mean of maximums
  return maxValues.reduce((sum, val) => sum + val, 0) / maxValues.length;
};

/**
 * First of Maximum defuzzification
 * @param {Object} fuzzyOutputs - Fuzzy rule outputs
 * @param {Object} outputMFs - Output membership functions
 * @param {Number} min - Minimum value in output range
 * @param {Number} max - Maximum value in output range
 ...
 * @returns {Number} - Defuzzified crisp value
 */
const defuzzifyFOM = (fuzzyOutputs, outputMFs, min, max, steps = 100) => {
  const stepSize = (max - min) / steps;
  let maxMembership = 0;
  let firstMaxValue = max;
